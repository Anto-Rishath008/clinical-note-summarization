...
def find_latest_checkpoint(checkpoint_dir: str) -> Optional[str]:
    ...
    # Match both regular and manual checkpoints
    checkpoints = glob.glob(os.path.join(checkpoint_dir, 'checkpoint_step_*.pt'))
    checkpoints += glob.glob(os.path.join(checkpoint_dir, 'manual_checkpoint_step_*.pt'))
    ...
    # Sort by step number (extract last number before .pt)
    def get_step(path):
        try:
    ...
    flush_print(f"  Latest checkpoint found: {os.path.basename(checkpoints[0])} (step {get_step(checkpoints[0])})")
...
def evaluate(
    ...
    for batch_idx, batch in enumerate(val_loader):
        ...
        generated = model.generate(
            src, src_mask,
            max_len=300,
            min_len=100,             # shorter eval generation for speed
...
def train(
    ...
    try:
      ...
      # Save checkpoint on any training error, then re-raise
      flush_print(f"\n[CRASH] Training error at step {global_step}: {type(train_err).__name__}: {train_err}")
      import traceback
      flush_print(traceback.format_exc())
      try:
          crash_path = os.path.join(training_config.checkpoint_dir, f'crash_checkpoint_step_{global_step}.pt')
          save_checkpoint_atomic(model, optimizer, scheduler, scaler, global_step, best_rouge_l, full_config, crash_path)
          flush_print(f"[CRASH] Saved crash checkpoint: {crash_path}")
      except Exception as save_err:
          flush_print(f"[CRASH] Failed to save crash checkpoint: {save_err}")
      raise
