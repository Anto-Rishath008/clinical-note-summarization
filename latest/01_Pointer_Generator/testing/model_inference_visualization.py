...
# =============================================================================
# Model Summary Function (similar to torchinfo style)
# =============================================================================

def get_layer_info(model):
    """
    Extract layer information from the Pointer-Generator model.
    Returns a list of tuples: (layer_name, output_shape, param_count)
    """
    layers_info = []
    
    # Helper to format shapes
    def format_shape(shape):
        return str(list(shape)).replace('[', '[-1, ').replace(']', ']', 1) if shape else "[-1]"
    
    # Encoder components
    encoder = model.encoder.encoder
    
    # Embedding layer
    emb_shape = encoder.embedding.weight.shape
    emb_params = encoder.embedding.weight.numel()
    layers_info.append(("Embedding (encoder)", f"[-1, seq_len, {emb_shape[1]}]", emb_params))
    ...
    layers_info.append((f"LSTM-{num_layers}L-Bi (encoder)", f"[-1, seq_len, {hidden_dim * 2}]", lstm_params))
    
    # Dropout (no params)
    layers_info.append(("Dropout (encoder)", f"[-1, seq_len, {hidden_dim * 2}]", 0))
    
    # Decoder components
    decoder = model.decoder
    
    # Decoder Embedding
    dec_emb_shape = decoder.embedding.weight.shape
    dec_emb_params = decoder.embedding.weight.numel()
    layers_info.append(("Embedding (decoder)", f"[-1, 1, {dec_emb_shape[1]}]", dec_emb_params))
    
    # Decoder LSTM
    dec_lstm = decoder.lstm
    dec_lstm_params = sum(p.numel() for p in dec_lstm.parameters())
    layers_info.append((f"LSTM-{dec_lstm.num_layers}L (decoder)", f"[-1, 1, {dec_lstm.hidden_size}]", dec_lstm_params))
    
    # Attention mechanism
    attn = decoder.attention
    attn_params = sum(p.numel() for p in attn.parameters())
    attn_type = "CoverageAttention" if hasattr(attn, 'W_c') else "AdditiveAttention"
    layers_info.append((f"{attn_type}", f"[-1, src_len]", attn_params))
    
    # Output projection
    out_proj_params = sum(p.numel() for p in decoder.out_proj.parameters())
    layers_info.append(("Linear (out_proj)", f"[-1, {decoder.vocab_size}]", out_proj_params))
    
    # Pointer-generator switch (if enabled)
    if decoder.p_gen_linear is not None:
        p_gen_params = sum(p.numel() for p in decoder.p_gen_linear.parameters())
        layers_info.append(("Linear (p_gen)", "[-1, 1]", p_gen_params))
    
    # Dropout decoder (no params)
    layers_info.append(("Dropout (decoder)", "[-1]", 0))
...
def print_model_summary(model, config):
    """
    Print a detailed model summary similar to torchinfo/torchsummary style.
    """
    print("\n" + "=" * 80)
    print(" " * 20 + "POINTER-GENERATOR MODEL SUMMARY")
    print("=" * 80)
    ...
    # Print header
    print(f"{'Layer (type)':<35} {'Output Shape':<25} {'Param #':>15}")
    print("=" * 80)
    
    total_params = 0
    for layer_name, output_shape, params in layers_info:
        print(f"{layer_name:<35} {output_shape:<25} {params:>15,}")
        total_params += params
    
    print("=" * 80)
    
    # Calculate actual totals from model
    actual_total = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    non_trainable = actual_total - trainable
    
    print(f"{'Total params:':<40} {actual_total:>15,}")
    print(f"{'Trainable params:':<40} {trainable:>15,}")
    print(f"{'Non-trainable params:':<40} {non_trainable:>15,}")
    print("-" * 80)
    
    # Estimate memory sizes
    param_size_mb = actual_total * 4 / (1024 ** 2)  # float32 = 4 bytes
    
    # Estimate input size (batch=1, typical input)
    chunk_len = config['model']['chunk_len']
    num_chunks = config['model']['num_chunks']
    max_src_len = chunk_len * num_chunks
    input_size_mb = max_src_len * 4 / (1024 ** 2)  # int32 tokens
    
    # Estimate forward/backward pass (rough: ~2x params for activations + gradients)
    forward_backward_mb = param_size_mb * 8  # Rough estimate
    
    total_size_mb = input_size_mb + forward_backward_mb + param_size_mb
    
    print(f"{'Input size (MB):':<40} {input_size_mb:>15.2f}")
    print(f"{'Forward/backward pass size (MB):':<40} {forward_backward_mb:>15.2f}")
    print(f"{'Params size (MB):':<40} {param_size_mb:>15.2f}")
    print(f"{'Estimated Total Size (MB):':<40} {total_size_mb:>15.2f}")
    print("-" * 80)
    
    # Model configuration summary
    print("\n" + "=" * 80)
    print(" " * 25 + "MODEL CONFIGURATION")
    print("=" * 80)
    print(f"{'Vocabulary Size:':<40} {config['data']['vocab_size']:>15,}")
    print(f"{'Embedding Dimension:':<40} {config['model']['emb_dim']:>15}")
    print(f"{'Hidden Dimension:':<40} {config['model']['hidden_dim']:>15}")
    print(f"{'Number of Layers:':<40} {config['model']['num_layers']:>15}")
    print(f"{'Chunk Length:':<40} {config['model']['chunk_len']:>15}")
    print(f"{'Number of Chunks:':<40} {config['model']['num_chunks']:>15}")
    print(f"{'Max Input Length:':<40} {max_src_len:>15}")
    print(f"{'Dropout:':<40} {config['model']['dropout']:>15}")
    print(f"{'Pointer-Generator:':<40} {'Enabled' if config['model'].get('pointer_gen', True) else 'Disabled':>15}")
    print(f"{'Coverage Mechanism:':<40} {'Enabled' if config['model']['use_coverage'] else 'Disabled':>15}")
    print("=" * 80)
    
    return {
        'total_params': actual_total,
...
def print_detailed_layer_breakdown(model):
    """
    Print an even more detailed layer-by-layer breakdown with all sub-modules.
    """
    print("\n" + "=" * 100)
    print(" " * 30 + "DETAILED LAYER BREAKDOWN")
    print("=" * 100)
    print(f"{'Layer':<50} {'Shape':<30} {'Parameters':>15}")
    print("=" * 100)
    ...
    for name, param in model.named_parameters():
        ...
        print(f"{display_name:<50} {shape_str:<30} {num_params:>15,}")
    
    print("=" * 100)
    print(f"{'TOTAL':<50} {'':<30} {total_params:>15,}")
    print("=" * 100)
...
def main():
    ...
    # =========================================================================
    # PRINT MODEL SUMMARY (similar to torchinfo style)
    # =========================================================================
    model_stats = print_model_summary(model, config)
    
    # Print detailed layer breakdown
    print_detailed_layer_breakdown(model)
